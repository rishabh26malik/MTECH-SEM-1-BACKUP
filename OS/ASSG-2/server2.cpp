...
int main(int argc, char *argv[])
{
	...
	processArgs(clients[clientNo], argv[1]);
	cout<<clients[clientNo].ip<<" "<<clients[clientNo].portNo<<endl;
	SetupServer(clients[clientNo], clientNo);
	while(1){
		len = sizeof(client_address);
		cout<<"B4 ACCEPT\n";
		clients[clientNo].sockID = accept(serverSkt, (struct sockaddr *)&server_address, &len);
		cout<<"AFER ACCEPT\n";
		if(client_skt < 0){
