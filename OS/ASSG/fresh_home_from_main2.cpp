...
void openDir(char path[], string newpath){
	...
	tmp.clear();
	if(path2list.find(newpath)==path2list.end()){	// changes made here, added hash cond. & used curr_idx(line - 130) instead of idx
		path2list[newpath]=idx;
		curr_idx=idx;
		idx++;	
	}else{
		curr_idx=path2list[newpath];
	}
	//path2list[newpath]=idx;
	//curr_idx=idx;
	//idx++;
	cout<<"\033c";//clear screen
	for(auto i : allDirList[curr_idx])
		cout<<i<<endl;
}
...
void changeDirectory(string newPath, int &start, int &end){
	...
	dir_len=allDirList[curr_idx].size();	// UPDATE curr_idx used instead of idx 
	if(newPath=="/home/rishabh/"){
		pwd="/home/rishabh/";
		curr_idx=0;
	}
	else
		pwd=newPath;
	start=0;
	row_len=size.ws_row-4;									// read terminal size here first---------to do
...
void removeFileEntry(string parent, string path){
	int index, flag=0;
	string filename="";
	...
	//string parentOfFile=path.substr(0,i+1);
	...
	//for(int i=0;i<allDirList[index].size();i++)
	//	cout<<allDirList[index][i]<<endl;
}
...
void deleteFile(vector <string> cmds){
	...
	while(i<n){
		string path=pwd+cmds[i];				// CORRECT IT------------------full path can also be given
		char Path[100];
		strcpy(Path, path.c_str());
		int fd=remove(Path);
		if(fd==-1){
	...
	while(1){
		...
		if(mode==1)		// normal mode
		{
			...
			if(key==27){
				...
				cout<<pwd+allDirList[idx-1][cursor_pos]<<endl;
...
12.) in opendir() & changeDirectory() -> correct hash, insert only if new directory
