...
void updatePermissions(char destn[]){
	struct stat dest;
	if (stat(destn, &dest) == -1) {
        perror("stat");
        exit(EXIT_FAILURE);
    }
    /*if( orig_file.st_mode & S_IRUSR )
    	dest.st_mode = dest.st_mode | 00400; 
    if( orig_file.st_mode & S_IRUSR )
    	dest.st_mode = dest.st_mode | 00200; 
    if( orig_file.st_mode & S_IRUSR )
    	dest.st_mode = dest.st_mode | 00100; 
    if( orig_file.st_mode & S_IRGRP )
    	dest.st_mode = dest.st_mode | 00040; 
    if( orig_file.st_mode & S_IXGRP )
    	dest.st_mode = dest.st_mode | ; 
    if( orig_file.st_mode & S_IWGRP )
    	dest.st_mode = dest.st_mode | ; 
    if( orig_file.st_mode & S_IROTH )
    	dest.st_mode = dest.st_mode | ; 
    if( orig_file.st_mode & S_IWOTH )
    	dest.st_mode = dest.st_mode | ; 
    if( orig_file.st_mode & S_IXOTH )
    	dest.st_mode = dest.st_mode | ;*/ 
    if( orig_file.st_mode & S_IRUSR )
    	dest.st_mode = dest.st_mode | S_IRUSR; 
    if( orig_file.st_mode & S_IWUSR )
    	dest.st_mode = dest.st_mode | S_IWUSR; 
    if( orig_file.st_mode & S_IXUSR )
		...
		while((fileInDir=readdir(directory))!=NULL){
			...
			if(fileName[0]!='.'){
				...
				if(File_OR_Dir(subPath)==2){	//file
					...
					searchRecusively(DirPath, fileName , isfile, found);
				}
			}
		}
		closedir(directory);
...
14.) goto 	------------------------------------------------------------------------------------------DONE
15.) on deleteing on file after another, 3rd/4th file gives "stat: No such file or directory" ERROR-BUT file is deleted - something wrong while displaying the directory
16.) update display if dir from pwd is deleted
17.) on delete_dir -> dir is deleted-> but "stat: No such file or directory" ERROR
-----------------------
		DONE
