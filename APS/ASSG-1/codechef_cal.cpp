...
void toPostfix(EXP *exp, PostFix **postfix){
	...
	while(exp!=NULL){
		...
		//cout<<operand<<" "<<isOperand<<endl;
		if( isOperand ){
			PostFixInsert(operand,false,postfix,&last);
			//cout<<(last->num)<<" inserted"<<endl;
		}
		else {
			//cout<<'a'<<" ";
			operatr=operand[0];
			if(S.empty() || operatr=='(' ){
				//cout<<operatr<<" pushed"<<endl;
				S.push(operatr);
			}
			else if ( operatr== ')'){
				while( S.top() != '(' ){
	...
	while(S.size>0){
	    ...
	    string st="";
		st+=S.top();
		PostFixInsert(st,true,postfix,&last);			
	    S.pop();
	    S.size--;
	}
	...
	//PostFixInsert("+",true,postfix,&last);
	//cout<<"after loop\n";
	/*PostFix *tmp2=*postfix;
	while(tmp2!=NULL){
		if( tmp2->isNum )
			cout<<tmp2->num<<" ";
		else
