...
public:
	...
	node* erase(node* root, T key)
	{
		...
		int rHeight = (root->right) ? root->right->height : 0;
		root->height= max(lHeight , rHeight) + 1;
		int bal_factor = getHeightDiff(root);

		int  bal_factor2 = getHeightDiff(root->left);
		if (bal_factor > 1 && bal_factor2 >= 0)
		{
			return LL_rotate(root);
		}
		if ((bal_factor > 1) && (bal_factor2 < 0))
		...
		bal_factor2 = getHeightDiff(root->right);
		if ((bal_factor < -1) && (bal_factor2 <= 0))
		{
			return RR_rotate(root);
		}
